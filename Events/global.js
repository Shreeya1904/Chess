...
import ModalHelper from "../Helper/modalCreator.js";
...
function checkForGameEnd() {
  if (!globalPiece.white_king.current_position) {
    ROOT_DIV.style.pointerEvents = "none";
    ModalHelper.showGameEndModal("Black wins! White king is captured.");
    return true;
  }
  if (!globalPiece.black_king.current_position) {
    ROOT_DIV.style.pointerEvents = "none";
    ModalHelper.showGameEndModal("White wins! Black king is captured.");
    return true;
  }
  return false;
}
...
function moveElement(piece, id, castle) {
  ...
  piece.current_position = id;
  if (pawnIsPromoted) {
    ModalHelper.pawnPromotion(inTurn, callbackPawnPromotion, id);
  }
  checkForCheck();
  if (checkForGameEnd()) {
    return;
...
// previous self highlight piece
let selfHighlightState = null;

// move state
let moveState = null;

// clear highlight local
